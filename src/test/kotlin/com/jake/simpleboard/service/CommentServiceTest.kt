...
class CommentServiceTest(
    ...
    given("댓글 생성시") {
        val post: Post = postRepository.save(Post(title = "제목", content = "내용", createdBy = "jake"))
        When("인풋이 정상적으로 들어오면") {
    ...
    given("댓글 수정시") {
        val post: Post = postRepository.save(Post(title = "제목", content = "내용", createdBy = "jake"))
        val saved: Comment = commentRepository.save(Comment(content = "댓글 내용", post, createdBy = "댓글 생성자"))
        When("인풋이 정상적으로 들어오면") {
            val updatedId: Long = commentService.updateComment(
                saved.id,
            ...
            then("정상 수정됨을 확인한다.") {
                ...
                val updated: Comment? = commentRepository.findByIdOrNull(updatedId)
    ...
    given("댓글 삭제시") {
        val post: Post = postRepository.save(Post(title = "제목", content = "내용", createdBy = "jake"))
        val saved: Comment = commentRepository.save(Comment(content = "댓글 내용", post, createdBy = "댓글 생성자"))
        val saved2: Comment = commentRepository.save(Comment(content = "댓글 내용2", post, createdBy = "댓글 생성자2"))
