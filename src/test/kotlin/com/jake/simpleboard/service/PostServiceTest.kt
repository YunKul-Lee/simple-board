...
class PostServiceTest(
    ...
    given("게시글 상세조회시") {
        ...
        likeService.createLike(saved.id, "jake")
        likeService.createLike(saved.id, "jake1")
        likeService.createLike(saved.id, "jake2")
        When("정상 조회시") {
    ...
    given("게시글 목록조회시") {
        ...
        When("좋아요가 2개 추가되었을때") {
            val postPage = postService.findPageBy(PageRequest.of(0, 5), PostSearchRequestDto(tag = "tag5"))
            postPage.content.forEach {
                likeService.createLike(it.id, "jake1")
                likeService.createLike(it.id, "jake2")
            }
